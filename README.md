...
### v2.1.0 - 即时奖励机制修复 (2025-11-28) 🎯

**核心修复：**
- 🔥 **修复即时奖励丢失bug** - 吃子、将军奖励现在会正确用于训练
- 🎯 即时奖励合理缩放（×0.01）避免压倒最终目标
- 🤖 混合训练模式（50%自我对弈 + 50%对抗旧模型）
- ⚙️ 优化MCTS动态配置（降低早期模拟次数，提升训练速度）

**新增功能：**
- 📋 **对局结束原因详细提示** - 显示"超过70步判和"、"将死红方"等详细原因
- 🎮 人机对战/观看模式显示结束原因
- 📊 评估模式显示结束原因
- 💾 备份旧模型用于对抗训练（models/old_opponent.pt）

**性能优化：**
- ⚡ MCTS早期模拟次数优化（1000-3000局: 100→35次，速度提升65%）
- 🔧 修复自我对弈时MCTS重复创建bug

**预期效果：**
- 🎯 预计 2000-2500 局出现首次胜负（之前1830局全是和棋）
- 📈 AI将学会吃子和将军策略
- 💪 训练效率大幅提升
