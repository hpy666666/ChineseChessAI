...
- ✅ **速度控制**: 5档速度调节 + 暂停/单步功能，方便观察每一步
- ✅ **进步曲线**: 自动绘制胜率、步数等训练指标，可视化AI成长
- ✅ **国规支持**: 实现三次重复判和、50回合无吃子、长将/长捉判负等规则
...
└── logs/                # 训练日志目录
    ...
    └── training_progress.png  # 进步曲线图（自动生成）
```
...
python plot_progress.py  # 查看训练进步曲线
```

### 观看AI对局时的控制

运行 `python main.py watch` 后，您可以使用以下按键控制：

| 按键 | 功能 | 说明 |
|------|------|------|
| **空格** | 暂停/继续 | 暂停对局以仔细观察当前局面 |
| **Enter** | 单步执行 | 每按一次走一步，方便逐步分析 |
| **1** | 极慢速度 | 3秒/步 |
| **2** | 慢速度 | 1.5秒/步 |
| **3** | 正常速度 | 0.5秒/步（默认） |
| **4** | 快速度 | 0.2秒/步 |
...
**使用技巧**：
- 想仔细检查每一步？按**空格**暂停，再按**Enter**单步观看
- AI下得太快？按**1**或**2**减速
- 只想快速看结果？按**5**加速
...
会显示4个图表：
1. **胜率变化趋势** - 红方、黑方、和局的胜率变化
2. **对局平均步数** - 看出AI是否学会快速取胜
3. **训练进度** - 累计训练了多少局
4. **最近一轮胜负分布** - 饼图显示最新表现

图表会自动保存到 `logs/training_progress.png`
